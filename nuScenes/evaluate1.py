...
if __name__ == "__main__":
    ...
    for ph in args.prediction_horizon:
        ...
        with torch.no_grad():
            ...
            eval_nll_batch_errors = np.array([])
            count=0
            ...
            for scene in tqdm(scenes):
                if count==1:
                    ...
                    predictions,co_matrix = eval_stg.predict(scene,
                                               timesteps,
                                               ph,
                                               num_samples=1,
                    ...
                    batch_error_dict = evaluation.compute_batch_statistics(predictions,
                                                                       ...
                                                                       kde=False)
                    #print("the shape of sigma_matrix is:")
                    ...
                    eval_nll_batch_errors = np.hstack((eval_fde_batch_errors, batch_error_dict[args.node_type]['nll']))
                count=count+1
            print('fde:{}/ade:{}/nll:{}'.format(np.mean(eval_fde_batch_errors), np.mean(eval_ade_batch_errors),np.mean(eval_nll_batch_errors)))
